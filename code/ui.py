...
class UI:
    '''
    Class dùng để tạo object UI và xử lý các tương tác của UI với game.
    Attributes:
        self.display_surface (pygame.Surface): Surface màn hình game
        self.font (pygame.font): font chữ chính của game
        #bar setup
        self.health_bar_rect (pygame.Rect): các thông số như độ cao, độ dài, vị trí cho health bar
        self.energy_bar_rect (pygame.Rect): các thông số như độ cao, độ dài, vị trí cho energy bar
    ...
    def show_bar(self, current, max_amount, bg_rect, color):
        ...
        Đồng thời cập nhật energy và health bar dựa trên số lượng người chơi đang có hiện tại.
        input:
            current (int): số lượng hiện tại
            max_amount (int): số lượng max
            bg_rect (pygame.Rect): rect background cho các thanh
            color (color): màu của thanh
        '''
    ...
    def show_exp(self,exp):
        '''
        Hàm vẽ số lượng exp người chơi đang sở hữu hiện tại
        input:
    ...
    def selection_box(self,left,top, has_switched):
        '''
        Hàm vũ khung lựa chọn mỗi khi thay đổi vũ khí/magic
        input:
